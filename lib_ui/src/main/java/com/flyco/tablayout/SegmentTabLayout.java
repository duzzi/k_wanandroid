...
public class SegmentTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    public void setTabDataRes(int[] titleResArray) {
        if (titleResArray == null || titleResArray.length == 0) {
            throw new IllegalStateException("Titles can not be NULL or EMPTY !");
        }

        String[] titles = new String[titleResArray.length];

        for (int i = 0; i < titleResArray.length; i++) {
            int titleRes = titleResArray[i];
            titles[i] = getResources().getString(titleRes);
        }
        setTabData(titles);
    }
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mGradientColors != null && mGradientColors.length > 0) {
            ...
            mIndicatorDrawable.setColors(mGradientColors);
        } else {
    ...
    public void switchTab(int tabIndex) {
        if (mCurrentTab != tabIndex) {
            ...
            if (mListener != null) {
                mListener.onTabSelect(tabIndex);
            }
        }
    }
    ...
    public void setIndicatorAndRelyOnColor(int indicatorColor) {
        this.mIndicatorColor = indicatorColor;
        setColorRelyOnIndicatorColor(indicatorColor);
        invalidate();
    }

    public void setBarStrokeColor(int indicatorColor) {
        mBarStrokeColor = indicatorColor;
        invalidate();
    }
    ...
    private void setColorRelyOnIndicatorColor(int indicatorColor) {
        mDividerColor = indicatorColor;
        mBarStrokeColor = indicatorColor;
        mTextUnselectColor = indicatorColor;
    }

    public void setBarStrokeWidth(float barStrokeWidth) {
        mBarStrokeWidth = dp2px(barStrokeWidth);
        invalidate();
    }
